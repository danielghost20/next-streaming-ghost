...
export function Thumbnail ({fallback, isLive, src, username}: ThumbnailProps) {
    ...
    if (!src) {
        ...
        content = (
            <Image className="object-cover transition-transform group-hover:translate-x-2 group-hover:-translate-y-2 rounded-md" src={src} fill alt="Thumbnail" />
        )
    }
    ...
    return (
        <div className="group aspect-video relative rounded-md cursor-pointer">
            <div className="rounded-md absolute inset-0 bg-blue-600 opacity-0 transition-opacity flex items-center justify-center group-hover:opacity-100"/>
                {content}
                {
              isLive && src && (
